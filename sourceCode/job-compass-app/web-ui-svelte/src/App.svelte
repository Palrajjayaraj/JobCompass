<script lang="ts">
  ...
  // Language options (derived from unique languages in jobs)
  let languageOptions = [
...
<div class="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-6">
  <div class="max-w-7xl mx-auto">
    <!-- Header -->
    <header class="mb-8">
      <h1 class="text-5xl font-bold text-white mb-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-600">
        🧭 JobCompass
      </h1>
      <p class="text-gray-400 text-lg">Discover your next opportunity with language-labeled job listings</p>
    </header>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      ...
      <div class="stat bg-base-200 shadow rounded-lg p-4">
        ...
        <div class="stat-value text-accent">{$jobs.filter(j => (j.applicationCount || 0) > 0).length}</div>
    ...
    <div class="bg-base-200 rounded-lg p-6 mb-6 shadow-xl">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <FilterDropdown
          label="Language"
          bind:value={$filters.selectedLanguage}
